          ...
          <div class="col-md-7 mt-4" style="top: 90px;left:18%">
            <div class="card line-chart-example" >
              <div class="card-header d-flex align-items-center">
                <h4 style="align: center">Select App Type</h4>
              </div>
           

              <form method="POST" action="{{route('app_type_submit')}}">
                @csrf
                <div class="form-group" style="padding: 18px">
                  
                  <select class="form-control select2" id="sel1" name="app_type">
                  <option selected disabled>Select Option</option>
                    @foreach($datas as $data)
                    <option value="{{$data->app_type}}">{{$data->app_type}}</option>
                     @endforeach
                  </select>
              </div>
